...
<html>
	...
	<body style="background-image: linear-gradient(purple,cyan);">
		...
		<?php
			...
			$query ="SELECT book_isbn,member,issue_id,issued_date, rdate FROM book_issue_log WHERE status = '0'";
			...
			else
			{
				...
				echo "<center><legend>Manage Returned BookRequests</legend></center>";
				...
				for($i=0; $i<$rows; $i++)
				{
					...
					echo "<td>
							<label class='control control--checkbox'>
								<input type='checkbox' name='cb_".$i."' value='".$row[2]."' />
				...
				echo "<input type='submit' value='Confirm' name='l_confirm' />&nbsp;&nbsp;&nbsp;";
			...
			if(isset($_POST['l_confirm']))
			{
				...
				for($i=0; $i<$rows; $i++)
				{
					if(isset($_POST['cb_'.$i]))
					{
						...
						$updsdf="SELECT book_isbn FROM book_issue_log WHERE issue_id='$id' LIMIT 1";
						$res=$con->query($updsdf);
						if (!$res){
							die(error_without_field("ERROR: Couldn\'t Confirm values"));
						}
						$isbno = mysqli_fetch_array($res)[0];

						$query3 = $con->prepare("SELECT copies FROM book WHERE isbn = ?;");
						$query3->bind_param("d", $isbno);
						$query3->execute();
						...
						$upddd="DELETE FROM book_issue_log WHERE issue_id='$id'";
						$update=$con->query($upddd);
						if (!$update){
							die(error_without_field("ERROR: Couldn\'t Confirm values"));
						}

						$upd = "UPDATE book SET copies='$qty' WHERE isbn = '$isbno'";
						$update=$con->query($upd);
						if (!$update){
							die(error_without_field("ERROR: Couldn\'t update "));
						}
				...
				if($members > 0){
					echo success("Successfully deleted ".$members." requests");
						header("location:pending_return_book_requests.php");
				}
				else{
			...
			if(isset($_POST['l_delete']))
			{
				...
				for($i=0; $i<$rows; $i++)
				{
					...
					{
						$id=$_POST['cb_'.$i];
						$upd="UPDATE book_issue_log SET status=1 WHERE issue_id='$id'";
						$update=$con->query($upd);
						if (!$update){
							die(error_without_field("ERROR: Couldn\'t Reject values"));
						}
						$requests++;
					}
					if($requests > 0){
